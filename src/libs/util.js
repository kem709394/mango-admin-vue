...
export const Dict_DATA_KEY = 'dict-data'
...
export const setDictData = (data) => {
  localStorage.setItem(Dict_DATA_KEY, JSON.stringify(data))
}

export const getDictData = () => {
  const data = localStorage.getItem(Dict_DATA_KEY)
  if (data) return JSON.parse(data)
  else return false
}
...
export const getBreadCrumbList = (route, homeRoute) => {
  ...
  let res = routeMetched.filter(item => {
    return item.meta && item.meta.bread
  }).map(item => {
    let meta = { ...item.meta }
...
export const getDictOptions = (code) => {
  const data = getDictData()
  if (data) {
    for (let i = 0; i < data.length; i++) {
      if (data[i].code === code) {
        return data[i].options
      }
    }
  }
  return []
...
export const tranDictName = (code1, code2) => {
  const options = getDictOptions(code1)
  for (let i = 0; i < options.length; i++) {
    if (options[i].code === code2) {
