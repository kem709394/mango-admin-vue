...
<template>
  <div>
    <Card>
      <p slot="title">
        <Icon type="ios-list-box"></Icon>内部消息
      </p>
      <Row>
        <Col span="12">
          <Input
        ...
        <Col span="12" style="text-align:right;">
          <Button v-if="$checkAccess('sys_message_create')" type="success" @click="handleCreate">添加</Button>
          <Button
            v-if="$checkAccess('sys_message_delete')"
            type="error"
    ...
    <Modal title="高级搜索" v-model="searchVisible" @on-ok="handleSearch">
      <Form ref="searchForm" :model="searchForm" :label-width="100">
        ...
        <FormItem label="类型">
          <Select v-model="searchForm.type" clearable style="width:300px">
            <Option
              v-for="(item,index) in $getDictOptions('MESSAGE_TYPE')"
              :value="item.code"
              :key="index"
            >{{ item.name }}</Option>
          </Select>
    ...
    <MessageCreate ref="createModal" @callback="getData"></MessageCreate>
    <MessageUpdate ref="updateModal" @callback="getData"></MessageUpdate>
    <MessageDetail ref="detailModal"></MessageDetail>
  </div>
</template>
<script>
import MessageCreate from "_c/sys/message/create";
import MessageUpdate from "_c/sys/message/update";
import MessageDetail from "_c/sys/message/detail";
...
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              "span",
              this.$tranDictName("MESSAGE_TYPE", params.row.type)
            );
          }
        },
        {
        ...
        },
        {
          ...
          render: (h, params) => {
            let array = [];
            if (this.$checkAccess("sys_message_detail")) {
              array.push(
                h(
                  "Button",
                  {
            ...
            if (this.$checkAccess("sys_message_update")) {
              array.push(
                h(
                  "Button",
                  {
            ...
            if (this.$checkAccess("sys_message_delete")) {
              array.push(
                h(
                  "Button",
                  {
  ...
  methods: {
    ...mapActions("sys/message", ["getData", "delete"]),
    ...mapMutations("sys/message", [
      "setPageNum",
      "setPageRows",
      "setPageFilter"
    ]),
    ...
    handleSearch() {
      ...
      if (this.searchForm.title.trim() !== "") {
        filter.title = this.searchForm.title.trim();
        this.searchTag.push("标题：" + this.searchForm.title.trim());
      }
      if (this.searchForm.type.trim() !== "") {
        filter.type = this.searchForm.type.trim();
        this.searchTag.push(
          "类型：" +
            this.$tranDictName("MESSAGE_TYPE", this.searchForm.type.trim())
        );
      }
